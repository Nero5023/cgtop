...
fn create_mock_cgroup_filesystem(temp_dir: &TempDir) -> PathBuf {
    ...
    fs::write(
        ...
        "8:0 rbytes=1024 wbytes=512 rios=10 wios=5\n",
    )
    .unwrap();
    ...
    fs::write(
        system_slice.join("cpu.stat"),
        "usage_usec 2000000\nuser_usec 1000000\nsystem_usec 400000\n",
    )
    .unwrap();
    fs::write(
        system_slice.join("io.stat"),
        "8:0 rbytes=2048 wbytes=1024 rios=20 wios=10\n",
    )
    .unwrap();
    ...
    fs::write(
        ssh_service.join("cpu.stat"),
        "usage_usec 500000\nuser_usec 300000\nsystem_usec 100000\n",
    )
    .unwrap();
    fs::write(
        ssh_service.join("io.stat"),
        "8:0 rbytes=512 wbytes=256 rios=5 wios=2\n",
    )
    .unwrap();
...
fn test_collect_cgroup_tree() {
    ...
    let system_path = cgroup_root
        .join("system.slice")
        .to_string_lossy()
        .to_string();
    let ssh_path = cgroup_root
        .join("system.slice/ssh.service")
        .to_string_lossy()
        .to_string();
