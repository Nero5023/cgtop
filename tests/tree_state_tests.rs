...
use common::{create_test_cgroup_paths, create_simple_cgroup_paths, create_mock_resource_stats};
...
fn test_state_persistence_across_updates() {
    ...
    paths.insert("/sys/fs/cgroup/test3".to_string(), common::create_mock_resource_stats());
    tree_state.build_from_paths(&paths);
...
fn test_complex_hierarchy() {
    ...
    assert!(system_slice.children.contains(&"system.slice/systemd-logind.service".to_string()));
    assert!(system_slice.children.contains(&"system.slice/ssh.service".to_string()));
    
    let user_slice = tree_state.nodes.get("user.slice").unwrap();
    assert!(user_slice.children.contains(&"user.slice/user-1000.slice".to_string()));
    
    // Check depth calculation
    let deep_node = tree_state.nodes.get("user.slice/user-1000.slice/user@1000.service/app.slice").unwrap();
...
fn test_node_path_normalization() {
    ...
    paths.insert("/sys/fs/cgroup/test".to_string(), create_mock_resource_stats());
