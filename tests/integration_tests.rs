...
fn test_app_metrics_update() {
    ...
    app.ui_state.tree_state.build_from_paths(&mock_metrics.resource_usage);
...
fn test_expansion_state_persistence() {
    ...
    // Expand a node
    let expandable_nodes: Vec<String> = app.ui_state.tree_state.nodes
        .iter()
    ...
    if let Some(node_to_expand) = expandable_nodes.first() {
        ...
        for expanded_path in &expanded_before {
            assert!(expanded_after.contains(expanded_path), "Expected {} to remain expanded", expanded_path);
        }
    }
}
...
fn test_ui_state_navigation() {
    ...
    // Build tree state
    ui_state.tree_state.build_from_paths(&mock_metrics.resource_usage);
...
fn test_cgroup_path_to_selection_sync() {
    ...
    if !app.ui_state.tree_state.visible_nodes.is_empty() {
        ...
        app.ui_state.selected_cgroup = app.ui_state.tree_state.selected.clone()
...
fn test_app_with_multiple_updates() {
    ...
    app.ui_state.tree_state.build_from_paths(&mock_metrics1.resource_usage);
    app.cgroup_data.metrics = Some(mock_metrics1);
    
    // Expand a node
    if !app.ui_state.tree_state.nodes.is_empty() {
        let first_expandable = app.ui_state.tree_state.nodes
            .iter()
    ...
    app.ui_state.tree_state.build_from_paths(&mock_metrics2.resource_usage);
    ...
    for expanded_path in &expanded_after_first {
        assert!(expanded_after_second.contains(expanded_path), "Expected {} to remain expanded", expanded_path);
    }
    
    // New cgroup should be present
    assert!(app.cgroup_data.metrics.as_ref().unwrap()
        .resource_usage.contains_key("/sys/fs/cgroup/new.slice"));
}